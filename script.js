...
document.addEventListener("DOMContentLoaded", () => {
 ...
 const studentListLabelEl = $("studentListLabel");
 ...
 // =========================================================
 // 4) Elements (Recruit inline form)
 // =========================================================
 ...
 // =========================================================
 // 5) Elements (Contact / SNS)
 // =========================================================
 const contactGrid = $("contactGrid");
 const snsGrid = $("snsGrid");
 ...
 // 8) Render students
 // ルール：
 // - 初期：今月の注目現役生（featured 最大2）
 // - 検索後：検索結果
 // =========================================================
 const setStudentsLabel = (text) => {
  if (!studentListLabelEl) return;
  studentListLabelEl.textContent = text || "";
 };

 const renderStudents = (list, opts = {}) => {
  ...
  if (label) setStudentsLabel(label);
   ...
   card.innerHTML = `
    <div class="studentTop">
     ...
     <div>
      ...
      <p class="studentMeta">${esc(stu.ageDecade || "")}</p>
     </div>
    </div>
 ...
 const renderFeatured = () => {
  const featured = students
   .filter((s) => !!s.featured)
   .slice(0, 2);
  renderStudents(featured, { label: "今月の注目現役生" });
 };
 ...
 const applyFilterAndJump = () => {
  ...
  renderStudents(filtered, { label: "検索結果" });
 ...
 if (recruitForm) {
  recruitForm.addEventListener("submit", (e) => {
   e.preventDefault();
   const mailto = makeRecruitMailto();
   if (!mailto) return;
   window.location.href = mailto;
  });
 } else if (recruitSendBtn) {
  // 予備：フォームが無い構成でも動くように
